...
CreateConVar("dm_music_volume", "50", FCVAR_ARCHIVE)
function MusicChecker()
	local ply = LocalPlayer()
	timer.Create("MusicChecker",5,0,function()
...
function RandomMusic()
	local ply = LocalPlayer()
	local vol = GetConVar( "dm_music_volume" ):GetInt()/100
	...
	if IsValid( ply.channel ) and ply.channel:GetState() != 0 then ply.channel:Stop() end
