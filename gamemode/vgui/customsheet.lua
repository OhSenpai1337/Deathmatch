...
derma.DefineControl( "CustomTab", "A Tab for use on the PropertySheet", PANEL, "DButton" )
...
function PANEL:AddSheet( label, panel, material, NoStretchX, NoStretchY, Tooltip )
	...
	Sheet.Tab = vgui.Create( "CustomTab", self )
	Sheet.Tab.Paint = function(self,w,h)
		draw.RoundedBox(0, 0, 0, w, h, Color (232, 232, 232, 255))
	end
	Sheet.Tab:SetTooltip( Tooltip )
	Sheet.Tab:Setup( label, self, panel, material )
	Sheet.Tab:SetTextColor(Color(0,0,0))

	Sheet.Panel = panel
	Sheet.Panel.NoStretchX = NoStretchX
	Sheet.Panel.NoStretchY = NoStretchY
	Sheet.Panel:SetPos( self:GetPadding()-5, 20 + self:GetPadding() )
...
derma.DefineControl( "CustomSheet", "", PANEL, "Panel" )
